...
import { MoreHorizontal, Eye, Printer, Pencil, Trash2, Plus, Search, Lock } from 'lucide-react';
import { Service } from '@/types/service.types';
import { getServices, updateServiceStatus, deleteService } from '@/lib/data/storage';
import { toast } from '@/components/ui/toast';
...
export function ServiceTable({ services: initialServices, onServiceDeleted }: ServiceTableProps) {
    ...
    const canEdit = (status: Service['status']) => {
    ...
    return (
        <div className="space-y-4">
                ...
                <Table>
                    ...
                    <TableBody>
                        {filteredServices.length === 0 ? (
                            ...
                            filteredServices.map((service) => {
                                const editable = canEdit(service.status);
                                return (
                                    <TableRow key={service.id} className="hover:bg-gray-50">
                                        <TableCell className="font-mono font-medium text-sm">
                                            {service.ticketNumber}
                                        ...
                                        <TableCell className="text-right">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    ...
                                                    {/* ✅ Editar - solo si está pendiente o en proceso */}
                                                    {editable ? (
                                                        <DropdownMenuItem onClick={() => router.push(`/services/${service.id}/edit`)}>
                                                            <Pencil className="h-4 w-4 mr-2" />
                                                            Editar
            ...
            {/* Añadir componente Tooltip si no existe */}
            <TooltipProvider />
        </div>
    );
}